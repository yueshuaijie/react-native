...
@property (nonatomic, copy) RCTDirectEventBlock onLoadingHeightChanged;
@property (nonatomic, assign) NSInteger contatinerIndex;
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    NSLog(@"webViewDidFinishLoad fittingSize:--%@", NSStringFromCGSize(fittingSize));
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"contentSize"]) {
        ...
        NSLog(@"observeValueForKeyPath fittingSize:--%@", NSStringFromCGSize(fittingSize));
        if (_onLoadingHeightChanged) {
            NSMutableDictionary<NSString *, id> *event = [self baseEvent];
            [event addEntriesFromDictionary:@{
                                              @"height": @(fittingSize.height),
                                              @"index": @(_contatinerIndex || 0)
                                              }];
            _onLoadingHeightChanged(event);
        }
    }
}
